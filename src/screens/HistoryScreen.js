...
const EMOJIS   = { CREATIVE:'🎨', LANDING_PAGE:'📄', COPY_GENERATION:'✍️', CAMPAIGN:'🚀', IDEA_VALIDATION:'💡', TRAFFIC:'📈', AB_TEST:'⚖️' };
const TYPE_CLR = { CREATIVE:'#ffb830', LANDING_PAGE:'#4f9fff', COPY_GENERATION:'#1fd97a', CAMPAIGN:'#ff5fa0', IDEA_VALIDATION:'#9b7dff', TRAFFIC:'#ff7a40', AB_TEST:'#0fd4b4' };
  ...
  return (
    <View style={styles.item}>
      ...
      <View style={{ flex: 1, marginLeft: 12 }}>
        <Text style={styles.itemTitle} numberOfLines={1}>{item.title || item.copyType}</Text>
      ...
      <View style={styles.itemRight}>
        {item.score != null && (
          <Text style={[styles.score, { color: item.score >= 8 ? COLORS.green : item.score >= 6 ? COLORS.amber : COLORS.red }]}>
            {item.score.toFixed(1)}
          </Text>
        )}
        <TouchableOpacity onPress={() => onFav(item)} style={styles.iconBtn}>
          <Ionicons name={item.isFavorite ? 'heart' : 'heart-outline'} size={18} color={item.isFavorite ? COLORS.pink : COLORS.text3} />
        </TouchableOpacity>
...
export default function HistoryScreen() {
  ...
  async function loadData(reset = false) {
    ...
    try {
      ...
      if (fRes.status === 'fulfilled') setFavorites(fRes.value.data?.data || []);
    } catch(e) {}
    setLoading(false);
  }
  ...
  async function handleFav(item) {
    ...
    setAnalyses(p => p.map(a => a.id === item.id ? { ...a, isFavorite: !a.isFavorite } : a));
  }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <HistoryItem item={item} onDelete={handleDelete} onFav={handleFav} />}
        contentContainerStyle={styles.list}
        showsVerticalScrollIndicator={false}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.accent2} />}
        onEndReached={() => { if (hasMore && tab === 0) { setPage(p => p + 1); loadData(); } }}
...
const styles = StyleSheet.create({
  ...
  itemIcon:       { width: 40, height: 40, borderRadius: RADIUS.sm, justifyContent: 'center', alignItems: 'center' },
  ...
  score:          { fontSize: 15, fontFamily: FONTS.heading, minWidth: 30, textAlign: 'right' },
  iconBtn:        { padding: 6 },
