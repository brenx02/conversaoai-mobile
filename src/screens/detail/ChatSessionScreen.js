...
const QUICK_PROMPTS = {
  COPYWRITER: [
    ...
    "Crie um hook poderoso",
    "Reescreva minha headline",
    "Técnica AIDA",
  ],
  TRAFFIC_MANAGER: [
    "Como reduzir meu CPA?",
    "Melhorar CTR Meta Ads",
    "Escalar campanha lucrativa",
      ...
      >
        <View style={styles.typingDots}>
          {[0, 1, 2].map((i) => (
            <View
              key={i}
              style={[styles.dot, { animationDelay: `${i * 200}ms` }]}
            />
          ))}
        </View>
      </View>
    ...
    <View style={isUser ? styles.userRow : styles.aiRow}>
      ...
      <View style={{ flex: 1, maxWidth: "85%" }}>
        ...
        <Text style={[styles.msgTime, isUser && { textAlign: "right" }]}>
          {new Date(item.createdAt || Date.now()).toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
          })}
        </Text>
...
export default function ChatSessionScreen() {
  ...
  useEffect(() => {
    ...
    aiAPI
      .createSession({ specialist: specialist.id })
  ...
  const renderItem = useCallback(
    ({ item }) => <Message item={item} onCopy={onCopy} />,
    [],
  );
